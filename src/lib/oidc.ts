/**
 * OIDC (OpenID Connect) integration — optional SSO via an external provider
 * (e.g. Pocket ID). Entirely disabled unless OIDC_ISSUER, OIDC_CLIENT_ID and
 * OIDC_CLIENT_SECRET are set.
 *
 * Single-user model: the first successful OIDC login links the provider
 * identity (issuer + subject) to the lone user_settings row; subsequent
 * logins must present the same subject.
 *
 * Uses openid-client v6: discovery is performed once and cached at module
 * level. The cross-redirect transaction (state/nonce/PKCE verifier) is
 * carried in a short-lived JWT cookie signed with SESSION_SECRET.
 */

import * as client from "openid-client";
import { SignJWT, jwtVerify } from "jose";
import { getSessionSecret } from "./constants";

export const OIDC_CALLBACK_PATH = "/api/auth/oidc/callback";
export const OIDC_TXN_COOKIE = "beez_oidc_txn";
export const OIDC_TXN_TTL_SECONDS = 600; // 10 minutes to complete the flow

/** True when all required OIDC env vars are present. */
export function isOidcConfigured(): boolean {
  return Boolean(
    process.env.OIDC_ISSUER &&
      process.env.OIDC_CLIENT_ID &&
      process.env.OIDC_CLIENT_SECRET
  );
}

/** Display name for the provider button ("Sign in with {name}"). */
export function getOidcProviderName(): string {
  return process.env.OIDC_PROVIDER_NAME || "SSO";
}

export function getOidcIssuer(): string {
  return process.env.OIDC_ISSUER || "";
}

// Module-level discovery cache. The promise is cached (not the resolved
// value) so concurrent first requests share one discovery round-trip. On
// failure the cache is cleared so the next request retries.
let configPromise: Promise<client.Configuration> | null = null;

export function getOidcConfiguration(): Promise<client.Configuration> {
  if (!isOidcConfigured()) {
    return Promise.reject(new Error("OIDC is not configured"));
  }
  if (!configPromise) {
    configPromise = client
      .discovery(
        new URL(process.env.OIDC_ISSUER!),
        process.env.OIDC_CLIENT_ID!,
        process.env.OIDC_CLIENT_SECRET!
      )
      .catch((err) => {
        configPromise = null;
        throw err;
      });
  }
  return configPromise;
}

/**
 * Derive the redirect URI. Prefers OIDC_REDIRECT_URI (set in production),
 * falling back to the origin of the incoming request.
 */
export function getRedirectUri(requestOrigin: string): string {
  return (
    process.env.OIDC_REDIRECT_URI ||
    new URL(OIDC_CALLBACK_PATH, requestOrigin).toString()
  );
}

/**
 * Build an absolute app URL for user-facing redirects. The request origin
 * inside the container is the bind address when running behind a reverse
 * proxy, so derive the public origin from OIDC_REDIRECT_URI when set.
 */
export function appUrl(path: string, requestUrl: string): URL {
  const base = process.env.OIDC_REDIRECT_URI
    ? new URL(process.env.OIDC_REDIRECT_URI).origin
    : requestUrl;
  return new URL(path, base);
}

/** Transaction state carried across the redirect in a signed cookie. */
export interface OidcTransaction {
  state: string;
  nonce: string;
  codeVerifier: string;
}

export async function sealOidcTransaction(
  txn: OidcTransaction
): Promise<string> {
  return new SignJWT({ ...txn })
    .setProtectedHeader({ alg: "HS256" })
    .setIssuedAt()
    .setExpirationTime(`${OIDC_TXN_TTL_SECONDS}s`)
    .sign(getSessionSecret());
}

export async function unsealOidcTransaction(
  sealed: string
): Promise<OidcTransaction | null> {
  try {
    const { payload } = await jwtVerify(sealed, getSessionSecret());
    if (
      typeof payload.state !== "string" ||
      typeof payload.nonce !== "string" ||
      typeof payload.codeVerifier !== "string"
    ) {
      return null;
    }
    return {
      state: payload.state,
      nonce: payload.nonce,
      codeVerifier: payload.codeVerifier,
    };
  } catch {
    return null;
  }
}

export { client as oidcClient };
