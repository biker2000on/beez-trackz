...
func Workbench(ctx context.Context, q app.Querier, actor app.Actor, year int, now time.Time, offline work.OfflinePredicate) (WorkbenchView, error) {
	...
	rows, err := q.Query(ctx, `
		WITH draft_lines AS (SELECT s.id sale_id,s.customer_name,COUNT(si.id) OVER(PARTITION BY s.id)::int line_count,si.item_id,si.inventory_lot_id,SUM(si.quantity) OVER(PARTITION BY s.id,si.item_id,si.inventory_lot_id,si.hive_id,si.kind)::int wanted,COALESCE(js.label,p.name,i.name,'item') label,CASE WHEN si.hive_id IS NOT NULL THEN (SELECT id FROM inventory_locations WHERE kind='deployed') ELSE COALESCE(mapped.id,home.id) END location_id,CASE WHEN si.kind='equipment' THEN 'serviceable' END condition,si.hive_id FROM sales s JOIN sale_items si ON si.sale_id=s.id LEFT JOIN jar_sizes js ON js.id=si.jar_size_id LEFT JOIN product_catalog p ON p.id=si.product_id LEFT JOIN inventory_items i ON i.id=si.item_id CROSS JOIN (SELECT id FROM inventory_locations WHERE is_home LIMIT 1) home LEFT JOIN inventory_locations mapped ON mapped.id=s.stock_location_id OR (mapped.source_type='stock_location' AND mapped.source_id=s.stock_location_id) WHERE s.physical_applied_at IS NULL AND s.order_status<>'cancelled' AND EXTRACT(YEAR FROM s.date)=$1), needs AS (SELECT DISTINCT sale_id,customer_name,line_count,item_id,inventory_lot_id,wanted,label,location_id,condition,hive_id FROM draft_lines WHERE item_id IS NOT NULL), avail AS (SELECT n.*,(COALESCE(SUM(a.available),0)+n.wanted)::int available FROM needs n LEFT JOIN inventory_available a ON a.item_id=n.item_id AND a.location_id=n.location_id AND a.condition IS NOT DISTINCT FROM n.condition AND a.container_hive_id IS NOT DISTINCT FROM n.hive_id AND (n.inventory_lot_id IS NULL OR a.lot_id=n.inventory_lot_id) GROUP BY n.sale_id,n.customer_name,n.line_count,n.item_id,n.inventory_lot_id,n.wanted,n.label,n.location_id,n.condition,n.hive_id) SELECT sale_id,customer_name,line_count,label,wanted,available FROM avail ORDER BY sale_id,label`, year)
	if err != nil {
		return WorkbenchView{}, app.Wrap(app.KindInternal, op, err)
	}
