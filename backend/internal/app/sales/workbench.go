...
func Workbench(ctx context.Context, q app.Querier, actor app.Actor, year int, now time.Time, offline work.OfflinePredicate) (WorkbenchView, error) {
	...
	rows.Close()
	rows, err = q.Query(ctx, `SELECT sl.id,sl.name,COALESCE(SUM(b.on_hand),0)::int,CASE sl.settlement_cadence WHEN 'weekly' THEN COALESCE(MAX(cs.period_end),CURRENT_DATE)+7 WHEN 'biweekly' THEN COALESCE(MAX(cs.period_end),CURRENT_DATE)+14 WHEN 'monthly' THEN (COALESCE(MAX(cs.period_end),CURRENT_DATE)+INTERVAL '1 month')::date WHEN 'quarterly' THEN (COALESCE(MAX(cs.period_end),CURRENT_DATE)+INTERVAL '3 months')::date ELSE NULL END,MAX(cs.period_end) FROM stock_locations sl LEFT JOIN inventory_locations il ON il.source_type='stock_location' AND il.source_id=sl.id LEFT JOIN inventory_balances b ON b.location_id=il.id LEFT JOIN consignment_settlements cs ON cs.location_id=sl.id AND cs.voided_at IS NULL WHERE sl.is_consignment AND sl.deleted_at IS NULL GROUP BY sl.id ORDER BY sl.name`)
	if err != nil {
		return WorkbenchView{}, app.Wrap(app.KindInternal, op, err)
	}
	for rows.Next() {
