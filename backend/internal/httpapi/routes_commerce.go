...
func (s *Server) mountCommerce(r chi.Router) {
	r.Get("/harvest-lots", s.harvestLotList)
	r.Post("/harvest-lots", s.harvestLotCreate)
	r.Get("/harvest-lots/{id}", s.harvestLotGet)
	r.Patch("/harvest-lots/{id}", s.harvestLotUpdate)
	r.Post("/harvest-lots/{id}/bottling-runs", s.bottlingRunCreate)
	r.Get("/harvest-lots/{id}/qr", s.harvestLotQR)

	r.Get("/expenses", s.expenseList)
	r.Post("/expenses", s.expenseCreate)
	r.Delete("/expenses/{id}", s.expenseDelete)
	r.Get("/customers", s.customerList)
	r.Post("/customers", s.customerCreate)
	r.Patch("/customers/{id}", s.customerUpdate)
	r.Get("/wholesale-price-lists", s.priceListList)
	r.Post("/wholesale-price-lists", s.priceListCreate)

	r.Get("/analytics/profitability", s.profitabilityAnalytics)
	r.Get("/analytics/economics", s.economicsAnalytics)
	r.Get("/honey/production-plan", s.productionPlan)
	r.Get("/honey/low-stock", s.lowStockAlerts)
	r.Get("/market-day/reconciliation", s.marketDayReconciliation)
	r.Get("/honey/sales/{id}/receipt", s.saleReceipt)
}
