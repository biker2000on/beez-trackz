...
func TestPostArtifactMigrationsMatchLegacySQL(t *testing.T) {
	files, err := filepath.Glob(filepath.Join("..", "db", "legacy-00001-00052", "0005[0-7]_*.sql"))
	if err != nil {
		t.Fatal(err)
	}
	if len(files) != 8 {
		t.Fatalf("migration files = %d, want 8: %v", len(files), files)
	}
	...
	if len(PostArtifactMigrations) != 8 {
		t.Fatalf("PostArtifactMigrations has %d entries, want one for each migration 00050-00057", len(PostArtifactMigrations))
	}
	...
	if !reflect.DeepEqual(got, want) {
		t.Fatalf("PostArtifactMigrations drifted from legacy SQL 00050-00057\n got: %#v\nwant: %#v", got, want)
	}
}
