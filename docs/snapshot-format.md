...
For every older artifact, the importer applies pending declarations in migration order. Migration 00056 leaves its five missing `inventory_locations` fields to become null on insert. Migration 00057 first derives one `user_preferences` record per `app_users` ID from the singleton `user_settings` values (without replacing a preference already present in the artifact), then removes `theme`, `default_apiary_id`, `date_format`, `weight_unit`, `units`, and `temperature_unit` from the `user_settings` insert. Migration 00058 removes `honey_variety` from the `harvest_lots` insert. Each applied transform is named in `restore-report.json`.
