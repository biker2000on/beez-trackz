...
| 00058 | 00005 | `harvest-lot-varietal-name-v1` | Removes the free-text `honey_variety` from `harvest_lots`; the varietal (`varietal_id`, already portable) is the lot's only name. The migration backfills a varietal for every lot whose text named none *on the migrating database*; a pre-00058 artifact restored onto a newer target simply drops the key, so such a lot restores with its `varietal_id` (possibly null) and no free text. |
| 00059 | 00006 | `harvest-lot-pulled-on-v1` | Adds nullable `pulled_on` (the day the frames were pulled) to `harvest_lots`. An older artifact's lot restores with it null. |
...
For every older artifact, the importer applies pending declarations in migration order. Migration 00056 leaves its five missing `inventory_locations` fields to become null on insert. Migration 00057 first derives one `user_preferences` record per `app_users` ID from the singleton `user_settings` values (without replacing a preference already present in the artifact), then removes `theme`, `default_apiary_id`, `date_format`, `weight_unit`, `units`, and `temperature_unit` from the `user_settings` insert. Migration 00058 removes `honey_variety` from the `harvest_lots` insert. Migration 00059 leaves the missing `harvest_lots.pulled_on` to become null on insert. Each applied transform is named in `restore-report.json`.
