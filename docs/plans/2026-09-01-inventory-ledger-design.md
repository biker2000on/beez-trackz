...
| GnuCash | `internal/httpapi/external_sync_rekey_test.go` | synthetic `external_sync` rows for all nine dissolved types re-key to `inventory_operation` via `legacy_ref`; `content_hash` rebaseline leaves unchanged remote bodies `synced`; a changed remote body stays `diverged` |
