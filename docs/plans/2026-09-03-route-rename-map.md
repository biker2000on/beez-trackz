...
**Status:** the machine-checkable half of
`docs/plans/2026-09-03-workflow-reset-design.md`. It is executed in one commit
by that document's **wave 5**. Old paths are **deleted**, never redirected
(roadmap: "Do not add compatibility redirects or preserve the current redirect
chain").
...
| # | Old path | New path | Type | Non-route consumers to update in the same commit |
|---|---|---|---|---|
| 1 | `/` | `/today` | move | `frontend/src/app/page.tsx:4` |
| 2 | `/dashboard` | `/today` | move | `sw.js/route.ts:28`; `install-prompt.tsx:35`; `nav-items.ts:45,381`; `manifest.ts:9` (`start_url`); `components/shell/sidebar.tsx:83`; `app/error.tsx:44`; `app/offline/page.tsx:24`; `features/apiaries/overview-tab.tsx:126`; `app/(auth)/login/page.tsx:63,87`; `features/transcription/batch-review.tsx:130`; `features/dashboard/dashboard-view.tsx` (deleted); **`backend/internal/httpapi/routes_auth.go:573`** (OIDC success redirect); e2e `design-promises.spec.ts:120`, `offline.spec.ts:54` |
| 3 | `/operations/yard-queue` | `/yard/queue` | move | `sw.js/route.ts:29`; `install-prompt.tsx:36`; `nav-items.ts:53,386`; `features/dashboard/dashboard-view.tsx:37`; `features/operations/yard-queue.tsx:135`; `features/operations/hooks.ts:189` (**API path — do not rename**, see §3); e2e `design-promises.spec.ts:121`, `offline.spec.ts:55` |
...
| 5 | `/apiaries` | `/yard/apiaries` | move | `install-prompt.tsx:37`; `nav-items.ts:61,382`; `manifest.ts:22` |
| 6 | `/apiaries/[id]` | `/yard/apiaries/[id]` | move | `nav-items.ts:246-247` (`contextualNavRoutes`); `provider.tsx:313-320`; e2e `navigation.spec.ts:161,166,226` |
| 7 | `/apiaries/[id]/flora` | `/yard/apiaries/[id]/flora` | move | `nav-items.ts:248` |
| 8 | `/apiaries/[id]/photos` | `/yard/apiaries/[id]/photos` | move | `nav-items.ts:249`; e2e `navigation.spec.ts:167` |
| 9 | `/apiaries/[id]/labels` | `/yard/apiaries/[id]/labels` | move | `nav-items.ts:250` |
| 10 | `/apiaries/[id]/bulk` | `/yard/apiaries/[id]/bulk` | move | `nav-items.ts:251` |
| 11 | `/apiaries/[id]/timeline` | `/yard/apiaries/[id]/timeline` | move | in-app links only |
| 12 | `/hives` | `/yard/hives` | move | `install-prompt.tsx:38`; `nav-items.ts:77,383`; `manifest.ts:29`; `sw.js/route.ts` `SHELL` (**added**, §2); e2e `a11y-bulk-select.spec.ts:72,98,116`, `navigation.spec.ts:263` |
| 13 | `/hives/[id]` | `/yard/hives/[id]` | move | `nav-items.ts:264-274`; `provider.tsx:334-341`; `features/dashboard/dashboard-view.tsx:174`; **`backend/internal/httpapi/yard_queue.go:104,169,212,265`**; **`backend/internal/httpapi/routes_field_intelligence.go:625`** (`hiveTagQR` target); e2e `navigation.spec.ts:169,174,226-228` |
| 14 | `/hives/[id]/equipment` | `/yard/hives/[id]/equipment` | move | `nav-items.ts:272` |
| 15 | `/hives/[id]/queen` | `/yard/hives/[id]/queen` | move | `nav-items.ts:273`; e2e `navigation.spec.ts:174-175` |
| 16 | `/hives/[id]/photos` | `/yard/hives/[id]/photos` | move | `nav-items.ts:274` |
| 17 | `/hives/[id]/transcribe` | `/yard/transcribe?hive=[id]` | delete | `nav-items.ts:276-280` (merged into one entry, S10) |
| 18 | `/transcribe` | `/yard/transcribe` | move | `nav-items.ts:68,254`; `features/apiaries/detail-page.tsx:153` |
| 19 | `/queens` | `/yard/queens` | move | `nav-items.ts:215,390`; `install-prompt.tsx` (**added**, currently missing) |
| 20 | `/genealogy` | — | delete-shim | `install-prompt.tsx:39` (stale entry, removed) |
| 21 | `/honey` | `/production` | move | `sw.js/route.ts:30`; `install-prompt.tsx:40`; `nav-items.ts:85,384`; `manifest.ts:36`; `features/honey/section-nav.tsx:27,60`; e2e `honey-gaps.spec.ts:116`, `navigation.spec.ts:181`, `design-promises.spec.ts:124` |
| 22 | `/honey/activity` | `/production/activity` | move | `nav-items.ts:92`; `section-nav.tsx:29`; e2e `navigation.spec.ts:188,243` |
| 23 | `/honey/production` | `/production/overview` | move | `nav-items.ts:97`; `section-nav.tsx:32` |
| 24 | `/honey/harvests` | `/production/harvests` | move | `nav-items.ts:100,108`; `section-nav.tsx:35` |
| 25 | `/honey/jars` | `/production/jars` | move | `nav-items.ts:101,109` |
| 26 | `/honey/lots` | `/production/lots` | move | `nav-items.ts:102,122`; `features/commerce/serial-lookup.tsx:144` |
| 27 | `/honey/serials` | `/production/serials` | move | `nav-items.ts:103,127`; `features/commerce/lots-tab.tsx:77`; `features/commerce/sale-serials.tsx:84` |
| 28 | `/honey/sessions/[id]` | `/production/sessions/[id]` | move | `nav-items.ts:103` (`matches`); `provider.tsx:363` |
| 29 | `/honey/products` | `/production/products` | move | `nav-items.ts:104,112` |
| 30 | `/honey/varietals` | `/production/varietals` | move | `nav-items.ts:105,117`; add to the new section-nav `matches` (today missing, §1.5 of the design doc) |
| 31 | `/honey/market-day` | — | delete-shim | `sw.js/route.ts:31`; `section-nav.tsx:47`; e2e `design-promises.spec.ts:123`, `honey-gaps.spec.ts:126`, `offline.spec.ts:58` |
| 32 | `/honey/sales` | — | delete-shim | none in-app (the **API** alias is separate, §3) |
| 33 | `/honey/sales/[id]` | — | delete-shim | none in-app |
| 34 | `/harvest` | — | delete-shim | none |
| 35 | `/harvest/activity` | — | delete-shim | none |
| 36 | `/harvest/harvests` | — | delete-shim | none |
| 37 | `/harvest/jars` | — | delete-shim | none |
| 38 | `/harvest/lots` | — | delete-shim | none |
| 39 | `/harvest/market-day` | — | delete-shim | none |
| 40 | `/harvest/production` | — | delete-shim | none |
| 41 | `/harvest/products` | — | delete-shim | none |
| 42 | `/harvest/sales` | — | delete-shim | none |
| 43 | `/harvest/sales/[id]` | — | delete-shim | none |
| 44 | `/harvest/serials` | — | delete-shim | none |
| 45 | `/harvest/sessions/[id]` | — | delete-shim | none |
| 46 | `/inventory` | `/equipment` | move | `install-prompt.tsx:41`; `nav-items.ts:156,387`; `features/dashboard/frame-shortage-widget.tsx:23`; nav keyword `nav-items.ts:161` drops the word "inventory" |
| 47 | `/inventory/types` | `/equipment/types` | move | `nav-items.ts:164` |
| 48 | `/reports` | `/insights` | move | `install-prompt.tsx:43`; `nav-items.ts:172,388`; `features/dashboard/dashboard-view.tsx:38`; `features/operations/reports-nav.tsx:19`; `features/operations/report-directory.tsx`; `features/operations/reports-overview.tsx`; e2e `navigation.spec.ts:198` |
| 49 | `/reports/outcomes` | `/insights/outcomes` | move | `nav-items.ts:178`; `reports-nav.tsx:32` |
| 50 | `/reports/survival` | `/insights/survival` | move | `nav-items.ts:181`; `reports-nav.tsx:21,34` |
| 51 | `/reports/yield` | `/insights/yield` | move | `nav-items.ts:182`; `reports-nav.tsx:22,35` |
| 52 | `/reports/finance` | `/insights/finance` | move | `nav-items.ts:187`; `reports-nav.tsx:37`; e2e `navigation.spec.ts:205,229` |
| 53 | `/reports/economics` | `/insights/economics` | move | `nav-items.ts:195`; `reports-nav.tsx:23,41` |
| 54 | `/reports/profitability` | `/insights/profitability` | move | `nav-items.ts:196`; `reports-nav.tsx:24,42` |
| 55 | `/reports/expenses` | `/sales/expenses` | move | `nav-items.ts:197`; `reports-nav.tsx:25,43` — **area change**, S11 |
| 56 | `/reports/sales-planning` | `/insights/sales-planning` | move | `nav-items.ts:202`; `reports-nav.tsx:47` |
| 57 | `/reports/bottling` | `/insights/bottling` | move | `nav-items.ts:206`; `reports-nav.tsx:26,49` |
| 58 | `/reports/customers` | `/sales/customers` | move | `nav-items.ts:207`; `reports-nav.tsx:27,49` — **area change**, S12 |
| 59 | `/settings` | `/me` + `/admin/setup` + `/admin` | delete | `nav-items.ts:231,391`; `features/settings/settings-view.tsx` split (wave 6); the compliance packet moves to `/insights/compliance` and the GnuCash reconciliation report to `/insights/reconciliation` |
| 60 | `/sales`, `/sales/[id]`, `/sales/market-day`, `/sales/consignment`, `/sales/consignment/[id]` | unchanged | — | `sw.js/route.ts:32-33` keeps both `SHELL` entries; `provider.tsx:352` unchanged |
