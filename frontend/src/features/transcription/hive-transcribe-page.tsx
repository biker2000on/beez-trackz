...
import { getHive } from "./api";
import { RecorderSheet } from "./audio-recorder";
import { SingleReviewPanel } from "./single-review";
import { TranscriptionStatusCard } from "./status-card";
import { useTranscriptionFlow } from "./use-transcription-flow";
 ...
 *
 * Reached as `/yard/transcribe?hive={id}` (design 2026-09-03 S10). The hive
 * id used to be a path segment on a route of its own; voice walkthrough and
 * voice inspection are one feature, so they are now one route.
 */
export function HiveTranscribePage({ hiveId: id }: { hiveId: string }) {
